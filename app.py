...
def create_pdf(data):
    ...
    pdf.set_text_color(*col_text)
    pdf.set_font("Arial", "", 11)
    pdf.cell(120, 8, txt("Gesamtausgaben (Pauschalen & Verpfl.):"), border='B')
    pdf.set_font("Arial", "B", 11)
...
# --- SIDEBAR ---
st.sidebar.header("1. Projekt-Daten & Nutzung")
...
# NEU: Quadratmeter für Bewirtschaftungskosten-Berechnung
wohnflaeche = st.sidebar.number_input(
    "Wohnfläche ca. (m²)", 
    value=defaults["wohnflaeche"], 
    step=10, 
    min_value=0, # FIX: Keine negative Fläche
    key="sb_wohnflaeche",
    help="Wichtig für die Schätzung der Nebenkosten (ca. 4€ pro m²)."
)
...
if nutzungsart == "Eigenheim (Nur Selbstbezug)":
    ...
    aktuelle_warmmiete = st.sidebar.number_input("Vergleich: Aktuelle Warmmiete", value=defaults["aktuelle_miete"], step=50, min_value=0, key="sb_akt_miete")
elif nutzungsart == "Eigenheim mit Vermietung (Einliegerw./MFH)":
    st.sidebar.success("ℹ️ ZUSÄTZLICHE Einnahmen durch Vermietung!")
    aktuelle_warmmiete = st.sidebar.number_input("Vergleich: Aktuelle Warmmiete", value=defaults["aktuelle_miete"], step=50, min_value=0, key="sb_akt_miete")
    neue_miete_einnahme = st.sidebar.number_input("Geplante Mieteinnahme (Kalt)", value=500, step=50, min_value=0, key="sb_neue_miete_mix")
elif nutzungsart == "Kapitalanlage (Reine Vermietung)":
    st.sidebar.warning("ℹ️ Alte Miete bleibt als Belastung.")
    aktuelle_warmmiete = st.sidebar.number_input("Aktuelle Warmmiete (bleibt)", value=defaults["aktuelle_miete"], step=50, min_value=0, key="sb_akt_miete")
    neue_miete_einnahme = st.sidebar.number_input("Geplante Mieteinnahme (Kalt)", value=600, step=50, min_value=0, key="sb_neue_miete_ka")
...
# INTELLIGENTE STANDARDS (BANK-LOGIK)
standard_lh = 1200 if anzahl_erwachsene == "Alleinstehend" else 1600
standard_kind = 400 * anzahl_kinder
standard_bewirtschaftung = wohnflaeche * 4 # ca. 4€ pro qm

with st.sidebar.expander("⚙️ Experten-Werte & Pauschalen", expanded=False):
    st.markdown("### Bank-Standards anpassen")
    var_kindergeld = st.number_input("Kindergeld (€)", value=250, step=10, min_value=0, key="exp_kindergeld")
    
    st.caption("Lebenshaltung (Essen, Kleidung, Gesundheit, Mobilität):")
    var_pauschale_erw = st.number_input(
        f"Pauschale Erwachsene (Std: {standard_lh}€)", 
        value=standard_lh, step=50, min_value=0, key="exp_p_erw",
        help="Enthält Lebensmittel, Kleidung, Mobilität, Versicherungen und Gesundheitskosten (z.B. Zuzahlungen Diabetes/Medikamente)."
    )
    var_pauschale_kind = st.number_input(
        f"Pauschale pro Kind (Std: 400€)", 
        value=400, step=25, min_value=0, key="exp_p_kind"
    )
    
    st.caption("Objektkosten:")
    var_bewirtschaftung = st.number_input(
        f"Bewirtschaftung Neu (Std: {standard_bewirtschaftung}€)", 
        value=float(standard_bewirtschaftung), step=10.0, min_value=0.0, key="exp_bewirt",
        help="Heizung, Strom, Wasser, Müll, Versicherung, Instandhaltung. Faustformel: 3,50€ - 4,50€ pro m²."
    )
    
    var_haircut_neu = st.number_input("Risikoabschlag Miete Neu (%)", value=80, step=5, max_value=100, min_value=0, key="exp_haircut_neu")
...
# FIX: min_value=0 bei allen Geldbeträgen
...
# Hier nutzen wir jetzt die variable Erwachsenen-Pauschale
gesamt_lebenshaltung = var_pauschale_erw + kinder_pauschale_gesamt
...
# JSON Speichern (mit neuen Feldern)
export_data = {
    "sb_name": kunden_name,
    "sb_nutzung": nutzungsart,
    "sb_wohnflaeche": wohnflaeche,
...
with col1:
    ...
    # NEU: LINKS ZU QUELLEN
    with st.expander("ℹ️ Quellen für Pauschalen anzeigen"):
        st.write("Dauerhafte Lebenshaltungskosten (Destatis):")
        st.markdown("[Statistisches Bundesamt - Konsumausgaben](https://www.destatis.de/DE/Themen/Gesellschaft-Umwelt/Einkommen-Konsum-Lebensbedingungen/Konsumausgaben-Lebenshaltungskosten/_inhalt.html)")
        st.write("Betriebskosten (Mieterbund):")
...
with col2:
    ...
    if freier_betrag < 0:
        ...
        # NEU: VERGLEICH MIETE vs EIGENTUM
        if aktuelle_warmmiete > 0 and nutzungsart != "Kapitalanlage (Reine Vermietung)":
            st.markdown("#### Vergleich: Miete vs. Eigentum")
            st.write(f"Alte Wohnkosten (Warm): {eur(aktuelle_warmmiete)}")
            # Neue Kosten = Rate + Bewirtschaftung (Nebenkosten) + Puffer
            neue_wohnkosten = freier_betrag + var_bewirtschaftung + puffer 
            # Hinweis: 'freier_betrag' ist die Rate. 
            
            diff_kosten = neue_wohnkosten - aktuelle_warmmiete
            if diff_kosten > 0:
                st.warning(f"📈 Mehrbelastung: Du musst monatlich **{eur(diff_kosten)}** mehr aufbringen als bisher.")
            else:
                st.success(f"📉 Ersparnis: Du zahlst rechnerisch **{eur(abs(diff_kosten))}** weniger als deine jetzige Miete.")
            st.markdown("---")
